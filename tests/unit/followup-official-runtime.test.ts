...
const send = vi.fn(async (..._args: unknown[]) => ({ kind: "sent", messageId: "msg-1" }));
const complete = vi.fn(async () => {});
const chain = vi.fn(async (input: RunBeforeSendArgs) => ({ status: "sent", outcome: await input.send(input.body) }));
vi.mock("@/lib/agent-engine/agent/inbound-turn", () => ({ runAgentTurn: agent, ritualBlocks: () => [] }));
...
function setup(provider: string, inbound: Date | null, payload: Record<string, unknown> = {}) {
  ...
  return { run: () => createFollowupTurnHandler(deps)(job, pool as never, { workerId: "worker-1" }), conversation };
}
