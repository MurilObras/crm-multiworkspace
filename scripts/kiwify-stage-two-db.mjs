...
const migration=readFileSync('supabase/migrations/20260921010000_0222_automation_action_identity.sql','utf8')+'\n'+readFileSync('supabase/migrations/20260921030000_0223_automation_event_plan.sql','utf8');
...
const catalog=`select jsonb_build_object(
 ...
 'constraints',(select jsonb_agg(to_jsonb(x) order by conname) from
  (select conname,pg_get_constraintdef(oid) definition from pg_constraint where conrelid in ('automation_rule_runs'::regclass,'followup_enrollments'::regclass)) x),
 'function',(select pg_get_functiondef('fn_redact_automation_destination()'::regprocedure))
) result`;
...
console.info('PASS: baseline install/reapply, main + 0222/reapply, colunas/constraints/função equivalentes');
