...
const row: KiwifyHistoryRow = {
  ...
  product_name:"Produto sintético",rule_name:"Aviso de compra",action_type:"send_whatsapp_message",action_index:0,
  channel:"synthetic",conversation_id:"conversation",message_id:"message",provider_id:"synthetic-provider",
  attempted_at:"2026-09-20T10:00:00Z",created_at:"2026-09-20T10:00:00Z",updated_at:"2026-09-20T10:01:00Z",status:"uncertain",reason:null,
};
...
it("mostra destinatário da tentativa separado do atual, aviso incerto e somente links existentes",async()=>{
  vi.stubGlobal("fetch",vi.fn(async()=>Response.json({data:{rows:[row],has_more:false}})));mount();
...
it("recusa de consentimento não sugere envio manual; ausência de dados não inventa cadastro",async()=>{
  vi.stubGlobal("fetch",vi.fn(async()=>Response.json({data:{rows:[{...row,status:"blocked",reason:"consent_declined",contact_name:null,current_phone:null,destination_phone:null,contact_id:null,conversation_id:null}],has_more:false}})));mount();
...
it("busca, situação, datas e paginação geram apenas consultas GET",async()=>{
  const fetch=vi.fn(async(..._args: unknown[])=>Response.json({data:{rows:[row],has_more:true}}));vi.stubGlobal("fetch",fetch);mount();
  ...
  expect(fetch.mock.calls.every(c=>!(c[1] as RequestInit | undefined)?.method)).toBe(true);
});
...
it("apresenta lista vazia",async()=>{
  vi.stubGlobal("fetch",async()=>Response.json({data:{rows:[],has_more:false}}));mount();
